...
//RTA VERSION
		//Times RTA old-school style. This can cause inconsitency due to your hardware (ie small freezes on crossing loadzones). 
		//Triggers internal splits when you enter the volume of a trigger, without care to what cycle that trigger is on.

//TODO LIST:
//add option to split at cutscene start instead of cutscene trigger (WIP)
//add option to use v3 logic for flooddown/up
//fix nadeboost floodup/down for legendary
...
vars.diffcheckdone = false;
vars.floodstate = 0;
return (current.tickcounter >= 0 && current.cutsceneskip != 1 && current.tickcounter < 15 && current.levelname == "d20"); //starts timer on specific tick that chief hits the ground
}
...
int checkindex = timer.CurrentSplitIndex + vars.indexoffset;
	switch (checkindex)
	{
		...
		return (current.trigx >= 1106794570 && current.trigy > 3212836864);
		break;
		...
		return (current.trigy <= 3266829312 && current.trigy > 3212836864);
		break;
		...
		return (current.trigx >= 1102276414 && current.trigy >= 3268097934);
		break;
		...
		return (current.trigy <= 3266394568 && current.trigy > 3212836864);
		break;
		...
		return (current.trigx <= 1072275462 && current.trigy > 3212836864);
		break;
		...
		return (current.trigy <= 3265091578 && current.trigy > 3212836864);
		break;
