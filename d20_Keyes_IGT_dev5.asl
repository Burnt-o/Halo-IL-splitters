...
state("halo") 
{
	int tickcounter1: 0x2F1D8C; //pauses on pause menu, resets on reverts
...
vars.lockedon = false;
vars.indexoffset = 0;
return (current.tickcounter1 >= 0 && current.cutsceneskip != 1 && current.tickcounter1 < 15 && current.levelname == "d20"); //starts timer on specific tick that chief hits the ground
}
